...
if __name__ == "__main__":
    ...
    basic_parser.add_argument('--plot-only', action='store_true')
    args_partial, _ = basic_parser.parse_known_args()

    if args_partial.plot_only:
        parser = argparse.ArgumentParser()
        parser.add_argument('--plot-only', action='store_true')
        parser.add_argument('--log-dir', dest='log_dir', required=True)
        parser.add_argument('--cperf-log', dest='cperf_log', default='cperf.log')
        parser.add_argument('--verbose', action='store_true')
        parser.add_argument('--old-slowdown', action='store_true')
        parser.add_argument('--tt-freeze', action='store_true')
        parser.add_argument('--dctcp', action='store_true')
        parser.add_argument('--skip', default=None)
        parser.add_argument('--unloaded', action='store_true')
        parser.add_argument('--ipv6', default="")
        parser.add_argument('--num-nodes', dest='num_nodes', type=int, default=2)
        parser.add_argument('--nodes', default="0,1")
        parser.add_argument('--mtu', type=int, default=0)
        parser.add_argument('--workload', default='')
        parser.add_argument('--client-max', type=int, default=200)
        parser.add_argument('--client-ports', type=int, default=3)
        parser.add_argument('--port-receivers', type=int, default=3)
        parser.add_argument('--server-ports', type=int, default=3)
        parser.add_argument('--port-threads', type=int, default=3)
    else:
        parser = get_parser("RTT analysis", usage="cperf.py", defaults={})
    ...
    if args.plot_only:
        ...
        # Generate digest and .data files
        digested = set()
        for file in os.listdir(args.log_dir):
            if file.endswith(".rtts"):
                exp_name = file.split("-")[0]
                if exp_name not in digested:
                    try:
                        get_digest(exp_name)
                        print(f"✅ Created .data for: {exp_name}")
                        digested.add(exp_name)
                    except Exception as e:
                        print(f"❌ Skipped {exp_name}: {e}")

        # Plot all experiments that were digested
        import matplotlib.pyplot as plt
        for exp in digested:
            ax = start_slowdown_plot(f"Slowdown: {exp}", 1000, exp)
            plot_slowdown(ax, exp, "p50", f"{exp} P50", color="blue")
            plot_slowdown(ax, exp, "p99", f"{exp} P99", color="green")
            plot_slowdown(ax, exp, "p999", f"{exp} P99.9", color="red")
            plt.legend()
            plot_path = os.path.join(args.log_dir, "reports", f"{exp}_slowdown.pdf")
